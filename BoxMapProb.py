...
def main_program():
	...
	if( type_sim == 2 ):
		...
		if( source_dem == 1 or source_dem == 3 ):
			[ matrix_compare , vertices_compare , polygon_compare , string_compare , data_direction ] = read_comparison_polygon_deg( comparison_polygon , comparison_type , input_file_cal , ang_cal , ang_cal_range , lon1 , lon2 , lat1 , lat2 , lon_cen , lat_cen , step_lat_m , step_lon_m , cells_lon , cells_lat , matrix_lon , matrix_lat , step_lon_deg , step_lat_deg , N )
		else:
			[ matrix_compare , vertices_compare , polygon_compare , string_compare , data_direction ] = read_comparison_polygon_utm( comparison_polygon , comparison_type , input_file_cal , ang_cal , ang_cal_range , east_cor , north_cor , east_cen , north_cen , cellsize , n_east , n_north , matrix_east , matrix_north , N )
	else:
		[ matrix_compare , vertices_compare , polygon_compare , string_compare , data_direction ] = [ np.nan , np.nan , np.nan , np.nan , np.nan ]
	...
	if( plot_flag == 1 and not np.isnan( data_cones ).any( ) ):
		if( source_dem == 1 or source_dem == 3 ):
			plot_deg( run_name , type_sim , Cities , polygon , lon1 , lon2 , lat1 , lat2 , step_lat_m , step_lon_m , matrix_lon , matrix_lat , lon_cen_vector , lat_cen_vector , volume_vector , phi_0_vector , ws_vector , Fr_vector , rho_p_vector , rho_gas_vector , Topography , Topography_Sea , N , data_cones , matrix_compare , ang_cal_range , data_direction , string_compare , comparison_polygon , comparison_type , vertices_compare )
		else:
